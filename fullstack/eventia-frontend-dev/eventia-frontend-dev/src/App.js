...
function App() {
  ...
  // FUNCIÓN PARA ACTUALIZAR CANTIDAD DE ITEMS EN EL CARRITO
  ...
  const handleCheckout = () => {
    if (cartItems.length === 0) {
      Swal.fire('Carrito vacío', 'Agrega eventos a tu carrito antes de finalizar la compra.', 'warning');
      return;
    }
  ...
  const handleConfirmPurchase = (paymentMethod) => {
    ...
    // Muestra mensaje de éxito según el método de pago
    if (paymentMethod === 'tarjeta') {
      Swal.fire({
        title: '¡Compra exitosa!',
        html: `
          <p>Tu pago con tarjeta ha sido procesado correctamente.</p>
          <p><strong>Total pagado: ${new Intl.NumberFormat('es-CL', { style: 'currency', currency: 'CLP', maximumFractionDigits: 0 }).format(finalTotal)}</strong></p>
          <p>Recibirás un correo con los detalles de tu compra y tus tickets.</p>
        `,
        icon: 'success',
      ...
      Swal.fire({
        ...
        html: `
          ...
          <p><strong>Total a pagar: ${new Intl.NumberFormat('es-CL', { style: 'currency', currency: 'CLP', maximumFractionDigits: 0 }).format(finalTotal)}</strong></p>
          <p>Acude a nuestras oficinas o puntos de venta autorizados con tu código QR para realizar el pago y recibir tus tickets.</p>
        `,
        icon: 'success',
    ...
    console.log("Compra finalizada con éxito. Método de pago:", paymentMethod);
  };
        ...
        <main>
          <Routes>
            ...
            <Route path="/carrito" element={<Cart cartItems={cartItems}
              onRemoveFromCart={handleRemoveFromCart} onCheckout={handleCheckout} onUpdateQuantity={handleUpdateQuantity} />} />
